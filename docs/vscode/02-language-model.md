...
要执行请求，请使用 `selectChatModels` 通过“供应商 (vendor)、ID、家族 (family) 或版本 (version)”进行过滤。当前可用模型家族包括 gpt-4o、gpt-4.1、gpt-5、gpt-5-mini、o3-mini、o4-mini、claude-3.5-sonnet、claude-sonnet-4、claude-sonnet-4.6、gemini-2.0-flash-001、gemini-2.5-pro 等。`sendRequest` 方法提供“基于流式”的响应，允许通过异步迭代器“连续报告结果和进度”。模型对象包含 `maxInputTokens` 属性；例如，GPT-4o 目前有 “64K” 的限制。
...
To execute a request, use `selectChatModels` to filter by "vendor, id, family, or version." Available model families now include gpt-4o, gpt-4.1, gpt-5, gpt-5-mini, o3-mini, o4-mini, claude-3.5-sonnet, claude-sonnet-4, claude-sonnet-4.6, gemini-2.0-flash-001, gemini-2.5-pro, etc. The `sendRequest` method provides a "streaming-based" response, allowing for "reporting results and progress continuously" via an async iterator. Model objects include a `maxInputTokens` attribute; for instance, GPT-4o currently has a "64K" limit.
...
自 `@types/vscode@1.110.0` 起，`LanguageModelDataPart` 已成为稳定 API。扩展可以使用 `LanguageModelDataPart.image()` 工厂方法在消息中嵌入图片数据：

```typescript
const imageData = new Uint8Array(/* ... base64 decoded bytes ... */);
const imagePart = vscode.LanguageModelDataPart.image(imageData, 'image/png');
const message = vscode.LanguageModelChatMessage.User([
    new vscode.LanguageModelTextPart('描述这张图片'),
    imagePart
]);
```

> **注意**: 并非所有模型都支持图片输入。可通过模型的 `capabilities.imageInput` 属性检查是否支持（需要提供者声明此能力）。在不支持的运行时上，`LanguageModelDataPart` 可能不可用，需要做运行时检测后降级。

## 可配置思考强度

对于支持推理 (reasoning) 的模型，可以通过请求选项控制思考强度。`LanguageModelThinkingPart` 目前仍为 proposed API，需运行时检测：

```typescript
const options: vscode.LanguageModelChatRequestOptions = {
    modelOptions: {
        thinkingBudget: 10000  // 控制模型思考 token 预算
    }
};
```

> **注意**: `LanguageModelThinkingPart` 尚未进入稳定类型面，使用时需通过 `(vscode as any).LanguageModelThinkingPart` 进行运行时探测。

## Interpret the response

After you've sent the request, you have to process the response from the language model API. The response (`LanguageModelChatResponse`) from the Language Model API is streaming-based. 流式响应的 `stream` 现在产出 `LanguageModelResponsePart` 联合类型，包括 `LanguageModelTextPart`、`LanguageModelToolCallPart` 和 `LanguageModelDataPart`。
